...
RUN pip3 install qmk && \
	apt-get update && \
	apt-get install --no-install-recommends -y git sudo wget && \
	apt-get install --no-install-recommends -y make gcc-arm-none-eabi gcc-avr avr-libc avrdude dfu-programmer dfu-util && \
	qmk setup -y && \
